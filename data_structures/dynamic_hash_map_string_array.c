...
int add_key_resize(static_hash_map* new_map, char* key, int* value){
    ...
    while (new_map->hash_map[hash_value].key != null_value && new_map->hash_map[hash_value].value[0] != nan_value){
        printf("KEY %s\n",new_map->hash_map[hash_value].key);
        printf("resize \n");
...
static_hash_map * resize_array(static_hash_map *map){
    ...
    for (int i=0; i < map->total_size; i++){
        printf("i%d: %s\n", i, map->hash_map[i].key);
        if (map->hash_map[i].key != null_value && map->hash_map[i].value[0] != nan_value){
            printf("resize key %s\n", map->hash_map[i].key);
...
int add_key_array(static_hash_map *map, char *key, int *value){
    ...
    while (map->hash_map[hash_value].key != null_value && map->hash_map[hash_value].value[0] != nan_value){
        printf("KEY %s\n",map->hash_map[hash_value].key);
